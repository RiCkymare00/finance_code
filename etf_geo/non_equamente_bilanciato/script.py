...
def portfolio_yearly_returns_weighted(which, months):
    w = random_weights(len(which))
    factors = dati[which] / dati[which].shift(months)
    weighted_factors = factors.dot(w)
    annual_return = weighted_factors.mean() ** (12 / months) - 1
    vol_monthly = weighted_factors.std()
    count = weighted_factors.count()

    return annual_return, vol_monthly, count, w
...
i = []
gpr_values = []
weights_list = []   # <-- lista per i pesi

for k in c:
    kk = list(k)
    gpr_sum, _ = calculate_gpr_sum(kk, gpr_data)
    if gpr_sum > 25:
        continue

    # Calcola ritorno, volatilità, conteggio e pesi
    ret, vol, cnt, w = portfolio_yearly_returns_weighted(kk, mesi)
    i.append(str(kk))
    r.append((ret, vol, cnt))
    gpr_values.append(gpr_sum)
    weights_list.append(w)   # <-- salva i pesi
...
results["Weights"] = weights_list  # <-- colonna dei pesi
